        ...
        for i in range(len(self.matModeSpace)):
            ...
            line_coef.append(
                torch.nn.Parameter(
                    scale
                    * torch.randn(
                        (1, self.static_only_components, 1, gridSize[mat_id_time_0])
                    )
                )
            )

        return torch.nn.ParameterList(plane_coef_space).to(device), \
